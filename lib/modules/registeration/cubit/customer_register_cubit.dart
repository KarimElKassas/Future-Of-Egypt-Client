...
class CustomerRegisterCubit extends Cubit<CustomerRegisterStates> {
  ...
  Future<void> insertLoginData(String userName,String userPassword, int userType) async {
    await DioHelper.postData(
        url: 'login/POST',
        query: {
      ...
      if (error is DioError) {
        emit(CustomerRegisterAddLoginDataErrorState(
            "لقد حدث خطأ ما برجاء المحاولة لاحقاً"));
      } else {
  ...
  void uploadUserFirebase(String userName, String userPhone, String userPassword, String userDocType, String userDocNumber, String userCity, String userRegion)async {
      ...
      FirebaseDatabase.instance.reference().child("Clients").child(userDocNumber).once().then((value){
        if(value.exists){
          isUserExist = true;
          emit(CustomerRegisterUserExistState());
        }else{
          FirebaseAuth.instance.createUserWithEmailAndPassword(
              email: "$userDocNumber@gmail.com",
              password: userPassword
          ).then((value) async {
            var token = await FirebaseMessaging.instance.getToken();
            saveUser(userName, userPhone, userPassword, userDocType, userDocNumber, userCity, userRegion, token!);
          }).catchError((error){
  ...
  Future saveUser(String userName, String userPhone, String userPassword, String userDocType, String userDocNumber, String userCity, String userRegion, String userToken) async {
    ...
    var storageRef = FirebaseStorage.instance.ref("Clients/$userDocNumber");
    FirebaseDatabase database = FirebaseDatabase.instance;
    var usersRef = database.reference().child("Clients");

    Map<String, Object> dataMap = HashMap();

    dataMap['ClientID'] = userDocNumber;
    ...
    dataMap["ClientLastMessageTime"] = "";
      ...
      usersRef.child(userDocNumber).set(dataMap).then((value) async {
        ...
        await uploadTask.then((p0) {
          p0.ref.getDownloadURL().then((value) {
            ...
            usersRef.child(userDocNumber).update(dataMap).then((realtimeDbValue)async {

              SharedPreferences prefs = await SharedPreferences.getInstance();

              prefs.setString("CustomerID", userDocNumber);
